...
void main(){
  ...
  // rote = tile(rote, 10. * uValueA + uValueB * roteA.x);

  // float dithering = (Bayer8(gl_FragCoord.xy  * .01  ) * 2.0 - 1.0) * 0.5;
  // float dithering2 = (Bayer2(gl_FragCoord.xy  * .01 ) * 2.0 - 1.0) * 0.5;
  // // uv.y += dithering;
  // // // coswarp2(uv, 3.);
  // // uv.x += dithering2;
  ...
  // uvRipple(uv, 1.5);
   ...
   // color= mix(color,vec3(square * uv.x, square2 *uv.y, cnoise(vec2(square3))), square);
   // //
    // coswarp(tex.rgb, 3. );
    // uv.y *= dithering;
